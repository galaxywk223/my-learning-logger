...
class Stage(db.Model):
    ...
    def to_dict(self): return {'id': self.id, 'name': self.name, 'start_date': self.start_date.isoformat(),
...
class WeeklyData(db.Model):
    ...
    def to_dict(self): return {'id': self.id, 'year': self.year, 'week_num': self.week_num,
                               'efficiency': self.efficiency, 'stage_id': self.stage_id}
...
class DailyData(db.Model):
    ...
    def to_dict(self): return {'id': self.id, 'log_date': self.log_date.isoformat(), 'efficiency': self.efficiency,
...
class CountdownEvent(db.Model):
    ...
    # --- 新增字段 ---
    # nullable=True 只是为了平滑过渡，后续我们会确保所有新数据都有值
